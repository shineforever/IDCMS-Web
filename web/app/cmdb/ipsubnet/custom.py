...
class CustomValidator():
    def __init__(self, item, item_id, value):
        ...
        self.sm =  {
            ...
            "expire_date": self.validate_expire_date
        }
    ...
    def validate_start_date(self, value):
        if re.match(re_date, value):
            ...
            expire_date = time.mktime(time.strptime(str(self.change_ipsubnet.expire_date),'%Y-%m-%d'))
    ...
    def validate_expire_date(self, value):
        if re.match(re_date, value):
            start_date = time.mktime(time.strptime(str(self.change_ipsubnet.start_date),'%Y-%m-%d'))
